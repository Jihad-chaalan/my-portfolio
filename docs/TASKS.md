...
- [x] **Breadcrumb deep-link verified:** the "All Projects" breadcrumb navigation works correctly in the current site flow.

## Task 4d — Reported bug: oversized project cards ✅ Complete

- [x] **Project cards too tall on desktop** — measured on 1440×900: card was 729px (**81% of the viewport**; 88% at 1366×768) because the fixed 16:9 image scaled with the 720px card width. Reduced in `ProjectCard.tsx` + `ProjectsShowcase.tsx`: card width 720px → 640px, image cap `max-h-[42svh]` → `max-h-[30svh]`, body padding `p-6 sm:p-7` → `p-6`, heading pinned at `text-2xl`, key features 3 → 2. Re-measured after the change: 580px @1440×900 (**64% of viewport**), 540px @1366×768 (**70%**). `tsc` / `eslint` / `next build` clean.
- [x] **Card sat too close to the title** — `centerTrack` measured its centering region from the track's start (which includes the title→track gap) and clamped the offset at 0, biasing the card toward the right edge on wide screens and crushing it against the title on narrow desktops. Now the region runs from the title column's right EDGE to the row's content edge, so free space is split evenly at any width — verified: title→card and card→edge gaps are both **100px** @1440. **User confirmed both fixes in the browser.**

## Task 5 — Quality ✅ Complete

- [x] Responsive pass across mobile/tablet/laptop/desktop.
- [x] Accessibility pass (heading order, landmarks, keyboard nav, contrast).
- [x] `app/not-found.tsx` custom 404.
- [x] Loading/error states where applicable.
- [x] Image optimization review — current SVG/local and Contentful asset images intentionally retain `unoptimized`; responsive `sizes` are defined and optimization can be enabled when raster screenshots replace the current assets.

## Task 6 — Production ⏳ In progress
